import mediapipe as mp #通常縮寫mp
# from mediapipe.tasks import python
# from mediapipe.tasks.python import vision

# 實際上工作的類別
GestureRecognizer = mp.tasks.vision.GestureRecognizer 
# 不同模型間都有的基礎辨識，eg: 模型路徑
...
options = GestureRecognizerOptions(
    base_options=BaseOptions(model_asset_buffer = model_file), # 直接讀模型的binary，丟給物件
    running_mode=VisionRunningMode.IMAGE)

with GestureRecognizer.create_from_options(options) as recognizer:
    
    # Load the input image from an image file.
    mp_image = mp.Image.create_from_file('images/victory1.jpg')
    # 手勢辨識
    gesture_recognition_result = recognizer.recognize(mp_image)
    # print resule
    top_gesture = gesture_recognition_result.gestures[0][0]
    hand_landmarks = gesture_recognition_result.hand_landmarks[0]
    print("top_gesture:", top_gesture.category_name, top_gesture.score)

    for landmark in hand_landmarks:
        print("landmarks:", round(landmark.x, 3), round(landmark.y, 3), round(landmark.z, 3))


# 目的:把拿到的手指節點(hand landmarks)，以及點跟點之間的連結，話在圖上
import cv2
...
# 把MediaPip(pillow)圖片 --> cv2 image (numpy array)
import numpy as np
