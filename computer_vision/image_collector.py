...
def put_cv2_text(image, text, org): #顯示出現在在做甚麼的資訊
    cv2.putText( 
        img=image,
        text=text,
...
def collect_image(folder_path, category_name, camera_id):
    ...
    while True:
        ...
        if is_success:
            ...
            put_cv2_text(show_frame, f"Collecting: {is_collection_start}", (30, 50)) # 顯示出目前有沒有在蒐集資料
            put_cv2_text(show_frame, f"Category: {category_name}", (30, 100)) # 顯示出蒐集資料的類別
    ...
    cv2.destroyWindow(window_name) # close all window

if __name__ == "__main__":
    camera_id = 0
    folder_path = "images/gesture"
    categories_name = ["open_plam", "thumb_up", "victory"]
